...
class ConversationManager:
    ...
    def get_conversation_state(self, call_id: str) -> Optional[Dict[str, Any]]:
        ...
        if not self.cache.exists(call_id):
            self.cache.set(call_id, {"ConversationIntent": ConversationIntent.WELCOME})
        
        state_data = self.cache.get(call_id)
        print(f"Conversation_state {state_data}")
        return state_data
    
    def update_conversation_state(self, call_id: str, new_state: ConversationIntent) -> bool:
        """Update specific fields in conversation state."""
        try:
            return self.cache.set(call_id, {"ConversationIntent": new_state})
